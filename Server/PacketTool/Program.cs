...
";
        ...
        " + Environment.NewLine + "\t";
                    break;
                case "list":
                    string cName = str.Split("\", \"")[1].Split("\" ]")[0];
                    string cNameLow = cName.Substring(0, 1).ToLower() + cName.Substring(1, cName.Length - 1);

                    if (varName.Equals("Class"))
                    {
                        read += "short " + cNameLow + @"CountSize = IPAddress.NetworkToHostOrder(BitConverter.ToInt16(buffer, offset));
        offset += sizeof(short);

        for (int i = 0; i < " + cNameLow + @"CountSize; i++)
        {
            short " + cNameLow + @"Size = IPAddress.NetworkToHostOrder(BitConverter.ToInt16(buffer, offset));
            offset += sizeof(short);
            " + cName + " " + cNameLow + " = new " + cName + @"();

            " + cNameLow + ".DeSerialize(new ArraySegment<byte>(buffer, offset + 2, " + cNameLow + @"Size - 2).ToArray());
            offset += " + cNameLow + @"Size;
            
            " + cNameLow + "List.Add(" + cNameLow + @");
        }
        ...
        offset += " + varName + "Pkt.Length;" + Environment.NewLine + Environment.NewLine + "\t";
                ...
                case "list":
                    #region 추가
                    string type = str.Split(": [ \"")[1].Split("\"")[0];
                    varName = str.Split("\", \"")[1].Split("\"")[0];
                    if (type.Equals("Class"))
                    {
                        bytesLen += " + " + varName + "CountSize.Length + (" + varName.Substring(0, 1).ToLower() + varName.Substring(1, varName.Length - 1) + "List[0].typeSizeSum + 2)" + " * " + varName.Substring(0, 1).ToLower() + varName.Substring(1, varName.Length - 1) + "List.Count";
                        result = "byte [] " + varName + "CountSize = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)" + varName.Substring(0, 1).ToLower() + varName.Substring(1, varName.Length - 1) + "List.Count));";
                        arrayCopy += "Array.Copy(" + varName + @"CountSize, 0, buffer, offset, " + varName + @"CountSize.Length);
        offset += " + varName + @"CountSize.Length;

        foreach (var item in " + varName.Substring(0, 1).ToLower() + varName.Substring(1, varName.Length - 1) + @"List)
        {
            byte[] " + varName + @"Pkt = item.Serialize();
            byte[] " + varName + @"Size = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)" + varName + @"Pkt.Length));
            ...
            offset += " + varName + @"Pkt.Length;
                    ...
                    {
                        bytesLen += " + " + varName + "CountSize.Length + sizeof(" + str.Split(": [ \"")[1].Split("\"")[0] + ") * " + varName.Substring(0, 1).ToLower() + varName.Substring(1, varName.Length - 1) + "List.Count";
                        result = "byte [] " + varName + "CountSize = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)" + varName.Substring(0, 1).ToLower() + varName.Substring(1, varName.Length - 1) + "List.Count));";
                        arrayCopy += "Array.Copy(" + varName + @"CountSize, 0, buffer, offset, " + varName + @"CountSize.Length);
        offset += " + varName + @"CountSize.Length;
        ...
        static string getVariable(string str)
        {
            ...
            switch (vari)
            {
                case "list":
                    string type = str.Split(": [ \"")[1].Split("\", \"")[0];
                    string listName = str.Split("\", \"")[1].Split("\" ]")[0];
                    if (type.Equals("Class"))
                    {
                        result = "public List<" + listName + "> " + listName.Substring(0, 1).ToLower() + listName.Substring(1, listName.Length - 1) + "List = new List<" + listName + ">();";
                    }
                    else
                    {
                        result = "public List<" + str.Split(": [ \"")[1].Split("\"")[0] + "> " + str.Split("\", \"")[1].Split("\"")[0] + "List = new List<" + str.Split(": [ \"")[1].Split("\"")[0] + ">();";
                    }
                    break;
                case "char":
                case "double":
